...
export function EditableCard({ game, isNewGame, onGameDelete, onToggleEdit, onUpdateGameList }: { game: Game, isNewGame?: boolean, onGameDelete: (id: string) => void, onToggleEdit?: () => void, onUpdateGameList: (game: Game) => void }) {
    ...
    return (
        <>
            {isNewGame ? <button className='btn btn-primary w-75 m-3' onClick={() => setIsOpen(true)}>Add game</button> : null}
            {isOpen ?
                <div className="card m-3 border-rounded">
                    ...
                    <div className="card-body">
                        <form>
                            <div className="mb-3">
                                <label htmlFor={'name-' + game.name} className="form-label">Name:</label>
                                <input id={"name-" + game.name} type="text" className="form-control" value={gameName} onChange={onGameNameChange} />
                            </div>
                            <div className="mb-3">
                                <label htmlFor={'description-' + game.name} className="form-label">Description:</label>
                            ...
                            <div className="form-check mb-3">
                                <label htmlFor={"has-goal-" + game.name} className="form-check-label">Has a goal</label>
                                <input id={"has-goal-" + game.name} type="checkbox" className="form-check-input" checked={hasGoal} onChange={onHasGoalChange} />
                            </div>
                            <div className="mb-3">
                                <label htmlFor={"goal-" + game.name} className="form-label">Goal:</label>
                            ...
                            <div className="btn-group m-3">
                                <div className="dropdown">
                                    <label className='form-label'>Playability assessment</label>
                                    <a
                            ...
                            <div className="mb-3">
                                <label htmlFor={"playability-justification-" + game.name} className="form-label">Justification of the playability assessment:</label>
                            ...
                            <div className="mb-3">
                                <label htmlFor={"content-validation-" + game.name} className="form-label">Content validation:</label>
                            ...
                            <div className="mb-3">
                                <label htmlFor={"observations-and-suggestions-" + game.name} className="form-label">Observations and suggestions:</label>
                            ...
                            <div className="mb-3">
                                <label htmlFor={"other-" + game.name} className="form-label">Other:</label>
                                <input id={"other-" + game.name} type="text" className="form-control" value={others} onChange={onOthersChange} />
                            </div>
                            <div className="mb-3">
                                <label htmlFor={"game-url-" + game.name} className="form-label">URL of the game:</label>
                                <input id={"game-url-" + game.name} type="text" className="form-control" value={link} onChange={onLinkChange} />
                            </div>
                            <div className="mb-3">
                                <label htmlFor={"image-url-" + game.name} className="form-label">URL of the image:</label>
                                <input id={"image-url-" + game.name} type="text" className="form-control" value={imageLink} onChange={onImageLinkChange} />
                            </div>
                            {!isNewGame ? <div><button type="button" className="btn btn-danger mb-3" onClick={handleDeleteClick}>Delete game</button></div> : null}
                            <div>
                                {!isNewGame ?
                                    <>
                                        <div className="mb-3"><button type="button" className="btn btn-primary" onClick={() => onGameUpdate()}>Save game</button></div>
                                        <div><button type="button" className="btn btn-secondary" onClick={onToggleEdit}>Close</button></div>
                                    </>
                                    :
                                    <>
                                        <div className="mb-3"><button type="button" className="btn btn-primary" onClick={() => onGameCreate(editedGame)}>Save game</button></div>
                                        <div><button type="button" className="btn btn-secondary" onClick={closePopup}>Close</button></div>
                                    </>
                                }
                            </div>
